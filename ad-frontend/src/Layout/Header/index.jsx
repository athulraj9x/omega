...
import { notifySuccess, playNotificationSound } from "../../utils/helper";
...
const Header = () => {
  ...
  // useLayoutEffect(() => {
  //   if(window.location.origin!==process.env.REACT_APP_BETXFAIR_ADMIN_DOMAIN){
  ...
  useEffect(()=>{
    const PlayDepositAudio=()=>{
      ...
      notifySuccess("Deposit Request Received")
    }
    if (whiteLabelType && whiteLabelType === "B2C") {
        socket.on("DepositRequestRaised", PlayDepositAudio)
      }
   
    return ()=>{
      socket.off("DepositRequestRaised",PlayDepositAudio );
    }
  }, [socket, whiteLabelType]);

  useEffect(()=>{
    const PlayWithdrawalAudio=()=>{       
      playNotificationSound("withdraw");
      notifySuccess("Withdraw Request Received");
    }
    if (whiteLabelType && whiteLabelType === "B2C") {
      socket.on("WithdrawRequestRaised", PlayWithdrawalAudio)
    }
    return ()=>{
      socket.off("WithdrawRequestRaised", PlayWithdrawalAudio );  
    }
  }, [socket, whiteLabelType]);
